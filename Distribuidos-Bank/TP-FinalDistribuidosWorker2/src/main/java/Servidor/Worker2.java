...
import java.util.Scanner;
	...
	public class Worker2{
		...
		private ServerSocket sSocket;
		    ...
		    private final static Logger log = LoggerFactory.getLogger(Worker2.class);
		    ...
		    public Worker2(int port) throws InterruptedException {
			    	
		        try {
		        	this.sSocket = new ServerSocket (port);
		        	log.info("Se ha iniciado el Worker en el puerto " + port );
		        	System.out.println("Se ha iniciado el Worker2 en el puerto " + port );
		        		...
		        		if(this.rWorker.getTransaccionOnQueue()) {//recuperamos una transaccion de la cola para ver si no esta vacia
		        			if(rWorker.getIntentosTransaccion() < umbral) {
				            	...
				            	while(!hilitos.isEmpty()) {
				                    ...
				                    while(j < hilitos.size() && !f) {
				                    	if (((RabbitTailWorker) hilitos.get(j)).getEncontre().equals("1")) {
				                    		f = true;
				                    		this.hash = ((RabbitTailWorker) hilitos.get(j)).getCadenaHash();
				                    		this.nonce = ((RabbitTailWorker) hilitos.get(j)).getNonce();
				        				    this.rWorker.setTransaccionBlockChain(( (RabbitTailWorker) hilitos.get(j)).getTransaccionBlockChain());
				        				    this. hilitos.removeAll(hilitos);
				        				}else {
				                        	if(((RabbitTailWorker) hilitos.get(j)).getEncontre().equals("0") || 
				                        			!hilitos.get(j).isAlive()){
				                           		contador++;
				                           		this.hilitos.remove(j);
				                           	}
				            			    if (contador == 5) {
				            			    	f = true;
				            				}
				            	...
				            	if(nonce.equals("-1")  || nonce.equals("0") ) {
				            		this.rWorker.sumarIntentos();
				            		this.rWorker.reEncolarTransaccion();
				            	}else {
				            		this.rWorker.encadenarTransaccion(nonce,hash);
				            	}
		    ...
		    private static int readPortConfigFile() {
				...
				Map config;
				try {
					config = gson.fromJson(new FileReader("serverWorker2Config.json"), Map.class);
					Map server = (Map) config.get("server");
