...
public class Empleado {
	...
	public void cerrarSession() {
		try {
			...
			Gson oG = new Gson();
			System.out.println("por cerrar app");
			this.op.setTipo("00");
	...
	public String validarEmpleado(String user, String psw) {
		try {
			this.s = new Socket ("127.0.0.1", 8888);//establecemos coneccion
			this.canalEntrada = new BufferedReader (new InputStreamReader (s.getInputStream()));
			this.canalSalida = new PrintWriter (s.getOutputStream(), true);
			this.og = new Gson();
			...
			op.setTipo("1");
			op.setUsuario(user);
			//fijarse entonces que al terminar la transaccion qeu sea, se elimine el hilo,con s.close(), agregar un thread.close
			op.setPsw(psw);
	...
	public String crearCuentaConTarjeta(String dni, String apellido, String nombre, String cuil, String fechaNacimiento, String calle,
			...
			try {
				...
				op.crearCuenta(dni, apellido, nombre, cuil, fechaNacimiento, calle, telefono,email,tipoTarjeta,tipoCuenta,
						provincia, localidad);
				this.json = og.toJson(op);
				this.canalSalida.println(json);
				this.canalSalida.flush();
	...
	//Obtiene los datos de un cliente como para la hora de modificar o eliminar un cliente
